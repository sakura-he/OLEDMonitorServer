...
function cpuTemperature(callback) {
    return new Promise((resolve) => {
        process.nextTick(() => {
            ...
            if (_linux) {
                const cmd = 'cat /sys/class/hwmon/hwmon0/device/hwmon/hwmon0/device/hwmon/hwmon0/temp*_la*;echo "---";cat /sys/class/hwmon/hwmon0/device/hwmon/hwmon0/device/hwmon/hwmon0/temp*_i*'
                exec(cmd, function(error, stdout) {
                    if (!error) {
                        let parts = stdout.toString().split('---')
                        let labels = parts[0].split('\n')
                        let temps = parts[1].split('\n')
                        console.log(parts, labels, temps)
                        temps.shift()
